...
public class BattleProcessEXP extends BattleProcessControl {
	...
	public synchronized void enemyReleaseEXP(FightPoklmon enemy) {
		...
		for (CustomScriptCall script : manager.getScriptCalls()) {
			if (script instanceof XpValueCalculationCallback) {
				expValue.value=((XpValueCalculationCallback) script).call(expValue.value);
			}
		}
	...
	private synchronized ArrayList<PlayerPoklmon> getGainingPoklmons(FightPoklmon enemy) {
		...
		for (CustomScriptCall script : manager.getScriptCalls()) {
			if (script instanceof XpReceiverCalculationCallback) {
				((XpReceiverCalculationCallback) script).call(poklmons);
			}
		}
		return poklmons;
	}
