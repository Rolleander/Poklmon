...
import com.broll.poklmon.game.items.callbacks.StatAttackCallback;
import com.broll.poklmon.game.items.callbacks.StatDefenceCallback;
import com.broll.poklmon.game.items.callbacks.StatInitiativeCallback;
import com.broll.poklmon.game.items.callbacks.StatKpCallback;
import com.broll.poklmon.game.items.callbacks.StatSpecialAttackCallback;
import com.broll.poklmon.game.items.callbacks.StatSpecialDefenceCallback;
...
public class FightPokemonBuilder {
	...
	public static void updateFightPoklmon(BattleManager battle, PlayerPoklmon pokl) {
		...
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatAttackCallback){
				v.value=((StatAttackCallback)script).call(v.value);
			}
		}
		pokl.getAttributes().setAttack(v.value);
		v.value = att.getDefence();
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatDefenceCallback){
				v.value=((StatDefenceCallback)script).call(v.value);
			}
		}
		pokl.getAttributes().setDefence(v.value);
		v.value = att.getInit();
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatInitiativeCallback){
				v.value=((StatInitiativeCallback)script).call(v.value);
			}
		}
		pokl.getAttributes().setInit(v.value);
		v.value = att.getSpecial_attack();
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatSpecialAttackCallback){
				v.value=((StatSpecialAttackCallback)script).call(v.value);
			}
		}
		pokl.getAttributes().setSpecial_attack(v.value);
		v.value = att.getSpecial_defence();
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatSpecialDefenceCallback){
				v.value=((StatSpecialDefenceCallback)script).call(v.value);
			}
		}
		pokl.getAttributes().setSpecial_defence(v.value);
		v.value = att.getMaxhealth();
		for(CustomScriptCall script: battle.getScriptCalls()){
			if(script instanceof StatKpCallback){
				v.value=((StatKpCallback)script).call(v.value);
			}
		}
